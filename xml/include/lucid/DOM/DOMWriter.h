...
public:
	...
	void setEncoding(const std::string& encodingName, Lucid::TextEncoding& textEncoding);
	...
	void writeNode(const std::string& systemId, const Node* pNode);
		...
		/// using a standard file output stream (Lucid::FileOutputStream).

private:
	std::string         _encodingName;
	Lucid::TextEncoding* _pTextEncoding;
	int                 _options;
	std::string         _newLine;
	std::string         _indent;
};
