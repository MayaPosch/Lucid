...
#include "lucid/DOM/AbstractContainerNode.h"
	...
	if (pNode && it != end)
	{
		if (*it == '[')
		{
			...
			else
			{
				...
				std::string idx;
				Lucid::UnicodeConverter::convert(index, idx);
				int i = Lucid::NumberParser::parse(idx);	
#else
				int i = Lucid::NumberParser::parse(index);
#endif
