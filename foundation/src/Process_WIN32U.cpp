...
#include "lucid/Process_WIN32U.h"
...
namespace
{
	std::vector<wchar_t> getUnicodeEnvironmentVariablesBuffer(const Lucid::Process::Env& env)
	{
		...
		for (const auto& p: env)
		{
			...
			std::wstring uname;
			Lucid::UnicodeConverter::convert(p.first, uname);
			std::wstring uvalue;
			Lucid::UnicodeConverter::convert(p.second, uvalue);
...
bool ProcessImpl::mustEscapeArg(const std::string& arg)
{
	...
	for (char c: arg)
	{
		if (Lucid::Ascii::isSpace(c) && !inQuotes && !escaped)
		{
			result = true;
			break;
		}
