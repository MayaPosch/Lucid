...
#include "lucid/NamedMutex_UNIX.h"
	...
	_name(name)
{
	...
	if (fd != -1)
		...
		throw SystemException(Lucid::format("cannot create named mutex %s (lockfile)", fileName), _name);
	...
	throw SystemException(Lucid::format("cannot create named mutex %s (semget() failed, errno=%d)", fileName, errno), _name);
