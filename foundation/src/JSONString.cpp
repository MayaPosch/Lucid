...
{
	bool wrap = ((options & Lucid::JSON_WRAP_STRINGS) != 0);
	bool escapeAllUnicode = ((options & Lucid::JSON_ESCAPE_UNICODE) != 0);
	...
	if(escapeAllUnicode)
	{
		std::string str = Lucid::UTF8::escape(value.begin(), value.end(), true);
	...
	else
	{
		for(std::string::const_iterator it = value.begin(), end = value.end(); it != end; ++it)
		{
			...
			{
				std::string str = Lucid::UTF8::escape(it, it + 1, true);
				(obj.*write)(str.c_str(), str.size());
			}else (obj.*write)(&(*it), 1);
		}
	}
...
{
	int options = (wrap ? Lucid::JSON_WRAP_STRINGS : 0);
