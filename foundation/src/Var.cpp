...
const Var Var::operator + (const Var& other) const
{
	...
	{
		if (isSigned())
			return add<Lucid::Int64>(other);
		else
			return add<Lucid::UInt64>(other);
	}
	else if (isNumeric())
...
Var& Var::operator += (const Var& other)
{
	...
	{
		if (isSigned())
			return *this = add<Lucid::Int64>(other);
		else
			return *this = add<Lucid::UInt64>(other);
	}
	else if (isNumeric())
...
const Var Var::operator - (const Var& other) const
{
	...
	{
		if (isSigned())
			return subtract<Lucid::Int64>(other);
		else
			return subtract<Lucid::UInt64>(other);
	}
	else if (isNumeric())
...
Var& Var::operator -= (const Var& other)
{
	...
	{
		if (isSigned())
			return *this = subtract<Lucid::Int64>(other);
		else
			return *this = subtract<Lucid::UInt64>(other);
	}
	else if (isNumeric())
...
const Var Var::operator * (const Var& other) const
{
	...
	{
		if (isSigned())
			return multiply<Lucid::Int64>(other);
		else
			return multiply<Lucid::UInt64>(other);
	}
	else if (isNumeric())
...
Var& Var::operator *= (const Var& other)
{
	...
	{
		if (isSigned())
			return *this = multiply<Lucid::Int64>(other);
		else
			return *this = multiply<Lucid::UInt64>(other);
	}
	else if (isNumeric())
...
const Var Var::operator / (const Var& other) const
{
	...
	{
		if (isSigned())
			return divide<Lucid::Int64>(other);
		else
			return divide<Lucid::UInt64>(other);
	}
	else if (isNumeric())
...
Var& Var::operator /= (const Var& other)
{
	...
	{
		if (isSigned())
			return *this = divide<Lucid::Int64>(other);
		else
			return *this = divide<Lucid::UInt64>(other);
	}
	else if (isNumeric())
		...
		default:
			{
				...
				{
					const Lucid::Int64 number = NumberParser::parse64(str);
					return number;
				}
				else if (frac == 0 && isNumber && !isSigned)
				{
					const Lucid::UInt64 number = NumberParser::parseUnsigned64(str);
					return number;
				}
...
std::string Var::parseJSONString(const std::string& val, std::string::size_type& pos)
{
	...
	if (!done) throw Lucid::DataFormatException("unterminated JSON string");
	return result;
}
