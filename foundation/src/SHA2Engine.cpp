...
typedef struct
{
	union
	{
		Lucid::UInt32 total32[4];
		Lucid::UInt64 total64[2];
	} total;

	union
	{
		Lucid::UInt32 state32[16];
		Lucid::UInt64 state64[8];
	} state;
...
static const Lucid::UInt64 K64[80] =
{
...
do {                                             \
	(n) = ( (Lucid::UInt32) (b)[(i)    ] << 24 )  \
		| ( (Lucid::UInt32) (b)[(i) + 1] << 16 )  \
		| ( (Lucid::UInt32) (b)[(i) + 2] <<  8 )  \
		| ( (Lucid::UInt32) (b)[(i) + 3]       ); \
} while( 0 )
#endif
#ifndef PUT_UINT32
...
{                                                \
	(n) = ( (Lucid::UInt64) (b)[(i)    ] << 56 )  \
		| ( (Lucid::UInt64) (b)[(i) + 1] << 48 )  \
		| ( (Lucid::UInt64) (b)[(i) + 2] << 40 )  \
		| ( (Lucid::UInt64) (b)[(i) + 3] << 32 )  \
		| ( (Lucid::UInt64) (b)[(i) + 4] << 24 )  \
		| ( (Lucid::UInt64) (b)[(i) + 5] << 16 )  \
		| ( (Lucid::UInt64) (b)[(i) + 6] <<  8 )  \
		| ( (Lucid::UInt64) (b)[(i) + 7]       ); \
}
#endif
#ifndef PUT_UINT64
...
{
	unsigned int i;
	Lucid::UInt32 temp1, temp2, temp3[8], W[64];
...
{
	int i;
	Lucid::UInt64 temp1, temp2, temp3[8], W[80];
...
void SHA2Engine::updateImpl(const void* buffer_, std::size_t count)
{
	...
	{
		left = (Lucid::UInt32)(pContext->total.total64[0] & 0x7F);
		size_t fill = 128 - left;
		pContext->total.total64[0] += (Lucid::UInt64)count;
		if (pContext->total.total64[0] < (Lucid::UInt64)count)	pContext->total.total64[1]++;
		if (left && count >= fill)
		{
	...
	{
		left = (Lucid::UInt32)(pContext->total.total32[0] & 0x3F);
		size_t fill = 64 - left;
		pContext->total.total32[0] += (Lucid::UInt32)count;
		pContext->total.total32[0] &= 0xFFFFFFFF;
		if (pContext->total.total32[0] < (Lucid::UInt32)count) pContext->total.total32[1]++;
		if (left && count >= fill)
		{
			memcpy((void *)(pContext->buffer + left), data, fill);
...
const DigestEngine::Digest& SHA2Engine::digest()
{
	...
	if (pContext->size > SHA_256)
	{
		...
		Lucid::UInt64 high = (pContext->total.total64[0] >> 61) | (pContext->total.total64[1] << 3);
		Lucid::UInt64 low = (pContext->total.total64[0] << 3);
	...
	else
	{
		...
		Lucid::UInt32 high = (pContext->total.total32[0] >> 29) | (pContext->total.total32[1] << 3);
		Lucid::UInt32 low = (pContext->total.total32[0] << 3);
