...
#include "lucid/DirectoryWatcher.h"
...
#include "lucid/Glob.h"
#include "lucid/DirectoryIterator.h"
...
public:
	...
	{
		_fd = inotify_init();
		if (_fd == -1) throw Lucid::IOException("cannot initialize inotify", errno);
	}
				...
				if (n > 0)
				{
					while (n > 0)
					{
						...
						{						
							...
							{
								Lucid::Path p(owner().directory().path());
								p.makeDirectory();
								p.setFileName(pEvent->name);
								Lucid::File f(p.toString());
...
public:
		...
		_stopped(false)
	{
		...
		if (_dirFD < 0) throw Lucid::FileNotFoundException(owner.directory().path());
		_queueFD = kqueue();
		if (_queueFD < 0)
		{
			close(_dirFD);
			throw Lucid::SystemException("Cannot create kqueue", errno);
		}
	}
...
DirectoryWatcher::DirectoryWatcher(const Lucid::File& directory, int eventMask, int scanInterval):
...
void DirectoryWatcher::init()
{
	if (!_directory.exists())
		throw Lucid::FileNotFoundException(_directory.path());
		
	if (!_directory.isDirectory())
		throw Lucid::InvalidArgumentException("not a directory", _directory.path());
