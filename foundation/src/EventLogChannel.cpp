...
void EventLogChannel::setUpRegistry() const
{
	...
	if (disp == REG_CREATED_NEW_KEY)
	{
		std::wstring path;
			...
			#if defined(_DEBUG)
				#if defined(_WIN64)
					path = findLibrary(L"LucidFoundation64d.dll");
				#else
					path = findLibrary(L"LucidFoundationd.dll");
				#endif
			#else
				#if defined(_WIN64)
					path = findLibrary(L"LucidFoundation64.dll");
				#else
					path = findLibrary(L"LucidFoundation.dll");
				#endif
			#endif
		#endif
		
		if (path.empty())
			path = findLibrary(L"LucidMsg.dll");
