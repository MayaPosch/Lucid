...
void AsyncChannel::log(const Message& msg)
{
	...
	if (_dropCount != 0)
	{
		_queue.enqueueNotification(new MessageNotification(Message(msg, Lucid::format("Dropped %z messages.", _dropCount))));
		_dropCount = 0;
	}
...
void AsyncChannel::setProperty(const std::string& name, const std::string& value)
{
	...
	{
		if (Lucid::icompare(value, "none") == 0 || Lucid::icompare(value, "unlimited") == 0 || value.empty())
			_queueSize = 0;
		else
			_queueSize = Lucid::NumberParser::parseUnsigned(value);
	}
	else
	{
