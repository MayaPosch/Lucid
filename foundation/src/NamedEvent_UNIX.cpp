...
#include "lucid/NamedEvent_UNIX.h"
	...
	_name(name)
{
	...
	if ((long) _sem == (long) SEM_FAILED)
		throw SystemException(Lucid::format("cannot create named mutex %s (sem_open() failed, errno=%d)", fileName, errno), _name);
#else
	int fd = open(fileName.c_str(), O_RDONLY | O_CREAT, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
	if (fd != -1)
		close(fd);
	else
		throw SystemException(Lucid::format("cannot create named event %s (lockfile)", fileName), _name);
	key_t key = ftok(fileName.c_str(), 'p');
	if (key == -1)
		throw SystemException(Lucid::format("cannot create named mutex %s (ftok() failed, errno=%d)", fileName, errno), _name);
	...
	else throw SystemException(Lucid::format("cannot create named mutex %s (semget() failed, errno=%d)", fileName, errno), _name);
