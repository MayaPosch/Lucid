...
void ThreadImpl::sleepImpl(long milliseconds)
{
	...
	Lucid::Timespan remainingTime(1000*Lucid::Timespan::TimeDiff(milliseconds));
	int rc;
	do
	{
		struct timespec ts;
		...
		{
			Lucid::Timestamp end;
			Lucid::Timespan waited = start.elapsed();
	...
	if (rc < 0 && remainingTime > 0) throw Lucid::SystemException("Thread::sleep(): nanosleep() failed");
#else
	Lucid::Timespan remainingTime(1000*Lucid::Timespan::TimeDiff(milliseconds));
	int rc;
	do
	...
	while (remainingTime > 0 && rc < 0 && errno == EINTR);
	if (rc < 0 && remainingTime > 0) throw Lucid::SystemException("Thread::sleep(): select() failed");
#endif
}
