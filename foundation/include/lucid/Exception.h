...
// Module:  Exception
//
// Definition of various Lucid exception classes.
//
...
#define POCO_DECLARE_EXCEPTION_CODE(API, CLS, BASE, CODE) \
	...
	public:																			\
		...
		CLS(const std::string& msg, const Lucid::Exception& exc, int code = CODE);	\
		CLS(const CLS& exc);														\
		~CLS() noexcept;																\
		CLS& operator = (const CLS& exc);											\
		const char* name() const noexcept;											\
		const char* className() const noexcept;										\
		Lucid::Exception* clone() const;												\
		void rethrow() const;														\
	};
...
#define POCO_IMPLEMENT_EXCEPTION(CLS, BASE, NAME)													\
	...
	CLS::CLS(const std::string& msg, const Lucid::Exception& exc, int code): BASE(msg, exc, code)	\
	{																								\
	}																								\
	...
	Lucid::Exception* CLS::clone() const																\
	{																								\
		return new CLS(*this);																		\
	}																								\
