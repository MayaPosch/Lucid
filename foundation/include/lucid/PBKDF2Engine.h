...
public:
	...
	PBKDF2Engine(const std::string& salt, unsigned c = 4096, Lucid::UInt32 dkLen = PRF_DIGEST_SIZE):
		_s(salt),
		_c(c),
		_dkLen(dkLen)
	{		
...
protected:
	...
	void f(Lucid::UInt32 i)
	{
		PRF prf(_p);
		prf.update(_s);
		Lucid::UInt32 iBE = Lucid::ByteOrder::toBigEndian(i);
		prf.update(&iBE, sizeof(iBE));
		Lucid::DigestEngine::Digest up = prf.digest();
		Lucid::DigestEngine::Digest ux = up;
		poco_assert_dbg(ux.size() == PRF_DIGEST_SIZE);
		for (unsigned k = 1; k < _c; k++)
		{
			prf.reset();
			prf.update(&up[0], up.size());
			Lucid::DigestEngine::Digest u = prf.digest();
