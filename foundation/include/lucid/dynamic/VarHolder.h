...
class Foundation_API VarHolder
	...
	/// std::string, Lucid::UTF16String, DateTime, LocalDateTime, Timestamp, std::vector<Var> and dynamicStruct.
	///
	/// Additional types can be supported by adding specializations. When implementing specializations,
	/// the only condition is that they reside in Lucid namespace and implement the pure virtual functions
	/// clone() and type().
	///
...
public:
	...
	virtual void convert(Lucid::UTF16String& val) const;
	/// Throws BadCastException. Must be overriden in a type
	/// specialization in order to support the conversion.
...
inline void VarHolder::convert(Lucid::UTF16String& /*val*/) const
{
	throw BadCastException("Can not convert to Lucid::UTF16String");
}
...
public:
	...
	void convert(Lucid::UTF16String& val) const
	{
		std::string str = NumberFormatter::format(_val);
		Lucid::UnicodeConverter::convert(str, val);
	}
...
public:
	...
	void convert(Lucid::UTF16String& val) const
	{
		Lucid::UnicodeConverter::convert(_val, val);
	}
...
{
public:
	VarHolderImpl(const char* pVal) : _val(Lucid::UnicodeConverter::to<UTF16String>(pVal))
	{
	}

	VarHolderImpl(const Lucid::UTF16String& val) : _val(val)
	{
	}
	...
	const std::type_info& type() const
	{
		return typeid(Lucid::UTF16String);
	}
	...
	const Lucid::UTF16String& value() const
	{
		return _val;
	}
	...
	UTF16Char& operator[](Lucid::UTF16String::size_type n)
	{
		if (n < size()) return _val.operator[](n);
	...
	const UTF16Char& operator[](Lucid::UTF16String::size_type n) const
	{
		if (n < size()) return _val.operator[](n);
...
public:
	...
	void convert(std::string& val) const
	{
		val = DateTimeFormatter::format(_val, Lucid::DateTimeFormat::ISO8601_FORMAT);
	}
