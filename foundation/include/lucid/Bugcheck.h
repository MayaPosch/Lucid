...
#define poco_bugcheck_msg(msg)          do { std::abort(); } while (0)
	...
	#define poco_assert_dbg(cond) \
		if (!(cond)) Lucid::Bugcheck::assertion(#cond, __FILE__, __LINE__); else (void) 0

	#define poco_assert_msg_dbg(cond, text) \
		if (!(cond)) Lucid::Bugcheck::assertion(#cond, __FILE__, __LINE__, text); else (void) 0
#else
	#define poco_assert_msg_dbg(cond, text)
	#define poco_assert_dbg(cond)
#endif
...
#define poco_check_ptr(ptr) \
	if (!(ptr)) Lucid::Bugcheck::nullPointer(#ptr, __FILE__, __LINE__); else (void) 0


#define poco_bugcheck() \
	Lucid::Bugcheck::bugcheck(__FILE__, __LINE__)


#define poco_bugcheck_msg(msg) \
	Lucid::Bugcheck::bugcheck(msg, __FILE__, __LINE__)
...
#define poco_unexpected() \
	Lucid::Bugcheck::unexpected(__FILE__, __LINE__);


#define poco_debugger() \
	Lucid::Bugcheck::debugger(__FILE__, __LINE__)


#define poco_debugger_msg(msg) \
	Lucid::Bugcheck::debugger(msg, __FILE__, __LINE__)
