...
class Foundation_API Logger: public Channel
	...
	/// Variants of these macros that allow message formatting with Lucid::format()
	/// are also available.
	///
	/// Examples:
...
public:
	...
	{
		log(Lucid::format(fmt, arg1, std::forward<Args>(args)...), Message::PRIO_FATAL);
	}
	...
	{
		log(Lucid::format(fmt, arg1, std::forward<Args>(args)...), Message::PRIO_CRITICAL);
	}
	...
	{
		log(Lucid::format(fmt, arg1, std::forward<Args>(args)...), Message::PRIO_ERROR);
	}
	...
	{
		log(Lucid::format(fmt, arg1, std::forward<Args>(args)...), Message::PRIO_WARNING);
	}
	...
	{
		log(Lucid::format(fmt, arg1, std::forward<Args>(args)...), Message::PRIO_NOTICE);
	}
	...
	{
		log(Lucid::format(fmt, arg1, std::forward<Args>(args)...), Message::PRIO_INFORMATION);
	}
	...
	{
		log(Lucid::format(fmt, arg1, std::forward<Args>(args)...), Message::PRIO_DEBUG);
	}
	...
	{
		log(Lucid::format(fmt, arg1, std::forward<Args>(args)...), Message::PRIO_TRACE);
	}
...
#define poco_fatal_f1(logger, fmt, arg1) \
	if ((logger).fatal()) (logger).fatal(Lucid::format((fmt), arg1), __FILE__, __LINE__); else (void) 0

#define poco_fatal_f2(logger, fmt, arg1, arg2) \
	if ((logger).fatal()) (logger).fatal(Lucid::format((fmt), (arg1), (arg2)), __FILE__, __LINE__); else (void) 0

#define poco_fatal_f3(logger, fmt, arg1, arg2, arg3) \
	if ((logger).fatal()) (logger).fatal(Lucid::format((fmt), (arg1), (arg2), (arg3)), __FILE__, __LINE__); else (void) 0

#define poco_fatal_f4(logger, fmt, arg1, arg2, arg3, arg4) \
	if ((logger).fatal()) (logger).fatal(Lucid::format((fmt), (arg1), (arg2), (arg3), (arg4)), __FILE__, __LINE__); else (void) 0

#define poco_fatal_f(logger, fmt, ...) \
	if ((logger).fatal()) (logger).fatal(Lucid::format((fmt), __VA_ARGS__), __FILE__, __LINE__); else (void) 0
...
#define poco_critical_f1(logger, fmt, arg1) \
	if ((logger).critical()) (logger).critical(Lucid::format((fmt), (arg1)), __FILE__, __LINE__); else (void) 0

#define poco_critical_f2(logger, fmt, arg1, arg2) \
	if ((logger).critical()) (logger).critical(Lucid::format((fmt), (arg1), (arg2)), __FILE__, __LINE__); else (void) 0

#define poco_critical_f3(logger, fmt, arg1, arg2, arg3) \
	if ((logger).critical()) (logger).critical(Lucid::format((fmt), (arg1), (arg2), (arg3)), __FILE__, __LINE__); else (void) 0

#define poco_critical_f4(logger, fmt, arg1, arg2, arg3, arg4) \
	if ((logger).critical()) (logger).critical(Lucid::format((fmt), (arg1), (arg2), (arg3), (arg4)), __FILE__, __LINE__); else (void) 0

#define poco_critical_f(logger, fmt, ...) \
	if ((logger).critical()) (logger).critical(Lucid::format((fmt), __VA_ARGS__), __FILE__, __LINE__); else (void) 0
...
#define poco_error_f1(logger, fmt, arg1) \
	if ((logger).error()) (logger).error(Lucid::format((fmt), (arg1)), __FILE__, __LINE__); else (void) 0

#define poco_error_f2(logger, fmt, arg1, arg2) \
	if ((logger).error()) (logger).error(Lucid::format((fmt), (arg1), (arg2)), __FILE__, __LINE__); else (void) 0

#define poco_error_f3(logger, fmt, arg1, arg2, arg3) \
	if ((logger).error()) (logger).error(Lucid::format((fmt), (arg1), (arg2), (arg3)), __FILE__, __LINE__); else (void) 0

#define poco_error_f4(logger, fmt, arg1, arg2, arg3, arg4) \
	if ((logger).error()) (logger).error(Lucid::format((fmt), (arg1), (arg2), (arg3), (arg4)), __FILE__, __LINE__); else (void) 0

#define poco_error_f(logger, fmt, ...) \
	if ((logger).error()) (logger).error(Lucid::format((fmt), __VA_ARGS__), __FILE__, __LINE__); else (void) 0
...
#define poco_warning_f1(logger, fmt, arg1) \
	if ((logger).warning()) (logger).warning(Lucid::format((fmt), (arg1)), __FILE__, __LINE__); else (void) 0

#define poco_warning_f2(logger, fmt, arg1, arg2) \
	if ((logger).warning()) (logger).warning(Lucid::format((fmt), (arg1), (arg2)), __FILE__, __LINE__); else (void) 0

#define poco_warning_f3(logger, fmt, arg1, arg2, arg3) \
	if ((logger).warning()) (logger).warning(Lucid::format((fmt), (arg1), (arg2), (arg3)), __FILE__, __LINE__); else (void) 0

#define poco_warning_f4(logger, fmt, arg1, arg2, arg3, arg4) \
	if ((logger).warning()) (logger).warning(Lucid::format((fmt), (arg1), (arg2), (arg3), (arg4)), __FILE__, __LINE__); else (void) 0

#define poco_warning_f(logger, fmt, ...) \
	if ((logger).warning()) (logger).warning(Lucid::format((fmt), __VA_ARGS__), __FILE__, __LINE__); else (void) 0
...
#define poco_notice_f1(logger, fmt, arg1) \
	if ((logger).notice()) (logger).notice(Lucid::format((fmt), (arg1)), __FILE__, __LINE__); else (void) 0

#define poco_notice_f2(logger, fmt, arg1, arg2) \
	if ((logger).notice()) (logger).notice(Lucid::format((fmt), (arg1), (arg2)), __FILE__, __LINE__); else (void) 0

#define poco_notice_f3(logger, fmt, arg1, arg2, arg3) \
	if ((logger).notice()) (logger).notice(Lucid::format((fmt), (arg1), (arg2), (arg3)), __FILE__, __LINE__); else (void) 0

#define poco_notice_f4(logger, fmt, arg1, arg2, arg3, arg4) \
	if ((logger).notice()) (logger).notice(Lucid::format((fmt), (arg1), (arg2), (arg3), (arg4)), __FILE__, __LINE__); else (void) 0

#define poco_notice_f(logger, fmt, ...) \
	if ((logger).notice()) (logger).notice(Lucid::format((fmt), __VA_ARGS__), __FILE__, __LINE__); else (void) 0
...
#define poco_information_f1(logger, fmt, arg1) \
	if ((logger).information()) (logger).information(Lucid::format((fmt), (arg1)), __FILE__, __LINE__); else (void) 0

#define poco_information_f2(logger, fmt, arg1, arg2) \
	if ((logger).information()) (logger).information(Lucid::format((fmt), (arg1), (arg2)), __FILE__, __LINE__); else (void) 0

#define poco_information_f3(logger, fmt, arg1, arg2, arg3) \
	if ((logger).information()) (logger).information(Lucid::format((fmt), (arg1), (arg2), (arg3)), __FILE__, __LINE__); else (void) 0

#define poco_information_f4(logger, fmt, arg1, arg2, arg3, arg4) \
	if ((logger).information()) (logger).information(Lucid::format((fmt), (arg1), (arg2), (arg3), (arg4)), __FILE__, __LINE__); else (void) 0

#define poco_information_f(logger, fmt, ...) \
	if ((logger).information()) (logger).information(Lucid::format((fmt), __VA_ARGS__), __FILE__, __LINE__); else (void) 0
	...
	#define poco_debug_f1(logger, fmt, arg1) \
		if ((logger).debug()) (logger).debug(Lucid::format((fmt), (arg1)), __FILE__, __LINE__); else (void) 0

	#define poco_debug_f2(logger, fmt, arg1, arg2) \
		if ((logger).debug()) (logger).debug(Lucid::format((fmt), (arg1), (arg2)), __FILE__, __LINE__); else (void) 0

	#define poco_debug_f3(logger, fmt, arg1, arg2, arg3) \
		if ((logger).debug()) (logger).debug(Lucid::format((fmt), (arg1), (arg2), (arg3)), __FILE__, __LINE__); else (void) 0

	#define poco_debug_f4(logger, fmt, arg1, arg2, arg3, arg4) \
		if ((logger).debug()) (logger).debug(Lucid::format((fmt), (arg1), (arg2), (arg3), (arg4)), __FILE__, __LINE__); else (void) 0

	#define poco_debug_f(logger, fmt, ...) \
		if ((logger).debug()) (logger).debug(Lucid::format((fmt), __VA_ARGS__), __FILE__, __LINE__); else (void) 0
	...
	#define poco_trace_f1(logger, fmt, arg1) \
		if ((logger).trace()) (logger).trace(Lucid::format((fmt), (arg1)), __FILE__, __LINE__); else (void) 0

	#define poco_trace_f2(logger, fmt, arg1, arg2) \
		if ((logger).trace()) (logger).trace(Lucid::format((fmt), (arg1), (arg2)), __FILE__, __LINE__); else (void) 0

	#define poco_trace_f3(logger, fmt, arg1, arg2, arg3) \
		if ((logger).trace()) (logger).trace(Lucid::format((fmt), (arg1), (arg2), (arg3)), __FILE__, __LINE__); else (void) 0

	#define poco_trace_f4(logger, fmt, arg1, arg2, arg3, arg4) \
		if ((logger).trace()) (logger).trace(Lucid::format((fmt), (arg1), (arg2), (arg3), (arg4)), __FILE__, __LINE__); else (void) 0

	#define poco_trace_f(logger, fmt, ...) \
		if ((logger).trace()) (logger).trace(Lucid::format((fmt), __VA_ARGS__), __FILE__, __LINE__); else (void) 0
#else
	#define poco_debug(logger, msg)
	#define poco_debug_f1(logger, fmt, arg1)
	#define poco_debug_f2(logger, fmt, arg1, arg2)
