...
public:
	...
	mutable Lucid::BasicEvent<bool> writable;
	...
	mutable Lucid::BasicEvent<bool> readable;
	...
	std::size_t peek(Lucid::Buffer<T>& buffer, std::size_t length = 0) const
		/// Peeks into the data currently in the FIFO
		/// without actually extracting it.
	...
	std::size_t read(Lucid::Buffer<T>& buffer, std::size_t length = 0)
		/// Copies the data currently in the FIFO
		...
		/// the used buffer size.
	{
		...
		if (length > available())
			throw Lucid::InvalidAccessException("Cannot extend buffer.");
		
		if (!isWritable())
			throw Lucid::InvalidAccessException("Buffer not writable.");
