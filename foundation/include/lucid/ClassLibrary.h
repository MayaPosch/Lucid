...
extern "C"
{
	bool POCO_LIBRARY_API pocoBuildManifest(Lucid::ManifestBase* pManifest);
...
extern "C"	\
{			\
	bool POCO_LIBRARY_API POCO_JOIN(pocoBuildManifest, name)(Lucid::ManifestBase* pManifest); \
}
...
#define POCO_BEGIN_MANIFEST_IMPL(fnName, base) \
	bool fnName(Lucid::ManifestBase* pManifest_)										\
	{																				\
		typedef base _Base;															\
		typedef Lucid::Manifest<_Base> _Manifest;									\
		std::string requiredType(typeid(_Manifest).name());							\
		std::string actualType(pManifest_->className());							\
		if (requiredType == actualType)												\
		{																			\
			Lucid::Manifest<_Base>* pManifest = static_cast<_Manifest*>(pManifest_);
...
#define POCO_EXPORT_CLASS(cls) \
    pManifest->insert(new Lucid::MetaObject<cls, _Base>(#cls));


#define POCO_EXPORT_INTERFACE(cls, itf) \
    pManifest->insert(new Lucid::MetaObject<cls, _Base>(itf));


#define POCO_EXPORT_SINGLETON(cls) \
	pManifest->insert(new Lucid::MetaSingleton<cls, _Base>(#cls));
