...
private:
	...
	public:
		...
		{
			val = static_cast<Lucid::Int32>(value);
			return *this;
		}

		Lucid::Int32 operator++()
		{
			return ++val;
		}
...
public:
		...
		/// exceeded, sender is notified of the current backlog size.
	{
		...
		try
		{
			...
			if (p)
			{
				Lucid::UInt16 off = handler().offset();
				...
				*pAL = SocketAddress::MAX_ADDRESS_LENGTH;
				...
				if (ret < 0)
				{
					...
					{
						Lucid::Int32 err = static_cast<Lucid::Int32>(errors);
						sock.sendTo(&err, sizeof(Lucid::Int32), SocketAddress(pSA, *pAL));
				...
				{
					Lucid::Int32 d = static_cast<Lucid::Int32>(data);
					sock.sendTo(&d, sizeof(Lucid::Int32), SocketAddress(pSA, *pAL));
