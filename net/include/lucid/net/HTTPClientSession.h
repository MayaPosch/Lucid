...
#include "lucid/Net/HTTPDigestCredentials.h"
#include "lucid/Net/HTTPNTLMCredentials.h"
...
public:
	...
	HTTPClientSession(const std::string& host, Lucid::UInt16 port = HTTPSession::HTTP_PORT);
		/// Creates a HTTPClientSession using the given host and port.

	HTTPClientSession(const std::string& host, Lucid::UInt16 port, const ProxyConfig& proxyConfig);
	...
	void setPort(Lucid::UInt16 port);
		/// Sets the port number of the target HTTP server.
		///
		/// The port number must not be changed once there is an
		/// open connection to the server.

	Lucid::UInt16 getPort() const;
		/// Returns the port number of the target HTTP server.

	void setProxy(const std::string& host, Lucid::UInt16 port = HTTPSession::HTTP_PORT);
		/// Sets the proxy host name and port number.

	void setProxyHost(const std::string& host);
		/// Sets the host name of the proxy server.

	void setProxyPort(Lucid::UInt16 port);
		/// Sets the port number of the proxy server.

	const std::string& getProxyHost() const;
		/// Returns the proxy host name.

	Lucid::UInt16 getProxyPort() const;
	...
	void setKeepAliveTimeout(const Lucid::Timespan& timeout);
		/// Sets the connection timeout for HTTP connections.

	const Lucid::Timespan& getKeepAliveTimeout() const;
	...
	virtual std::istream& receiveResponse(HTTPResponse& response);
		...
		/// in a Lucid::Net::MessageException being thrown.
		///
		/// In case a network or server failure happens
...
private:
	...
	Lucid::Timestamp _lastRequest;
	bool            _reconnect;
	bool            _mustReconnect;
	bool            _expectResponseBody;
	bool            _responseReceived;
	Lucid::SharedPtr<std::ostream> _pRequestStream;
	Lucid::SharedPtr<std::istream> _pResponseStream;
...
inline Lucid::UInt16 HTTPClientSession::getPort() const
{
	return _port;
}
...
inline Lucid::UInt16 HTTPClientSession::getProxyPort() const
{
...
inline const Lucid::Timespan& HTTPClientSession::getKeepAliveTimeout() const
{
