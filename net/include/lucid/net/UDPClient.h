...
class Net_API UDPClient : public Lucid::Runnable
	...
	/// separate thread.
	/// This class is written as a "companion" to Lucid::Net::UDPServer.
	/// For other servers, inherit from this class and override the
	/// handleResponse(char*, int) virtual member.
{
public:
	UDPClient(const std::string& address, Lucid::UInt16 port, bool listen = false);
...
private:
	...
	Thread*             _pThread;
	bool                _stop;
	Lucid::AtomicCounter _dataBacklog;
	Lucid::AtomicCounter _errorBacklog;
};
