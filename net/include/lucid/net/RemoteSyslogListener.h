...
#include "lucid/NotificationQueue.h"
...
class Net_API RemoteSyslogListener: public Lucid::SplitterChannel
	...
	///
	/// The RemoteSyslogListener is a subclass of Lucid::SplitterChannel.
	/// Every received log message is sent to the channels registered
	/// with addChannel() or the "channel" property.
	///
	/// Lucid::Message objects created by RemoteSyslogListener will have
...
public:
	...
	RemoteSyslogListener(Lucid::UInt16 port);
		/// Creates the RemoteSyslogListener, listening on the given port number.

	RemoteSyslogListener(Lucid::UInt16 port, int threads);
		/// Creates the RemoteSyslogListener, listening on the given port number
	...
	void enqueueMessage(const std::string& messageText, const Lucid::Net::SocketAddress& senderAddress);
