...
#include "lucid/URIStreamFactory.h"
...
class Net_API HTTPStreamFactory: public Lucid::URIStreamFactory
...
public:
	...
	HTTPStreamFactory(const std::string& proxyHost, Lucid::UInt16 proxyPort = HTTPSession::HTTP_PORT);
		/// Creates the HTTPStreamFactory.
		///
		/// HTTP connections will use the given proxy.

	HTTPStreamFactory(const std::string& proxyHost, Lucid::UInt16 proxyPort, const std::string& proxyUsername, const std::string& proxyPassword);
		/// Creates the HTTPStreamFactory.
		///
	...
	virtual std::istream* open(const Lucid::URI& uri);
