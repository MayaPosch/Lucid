...
class Net_API HTTPCredentials
	...
	///     Lucid::Net::HTTPCredentials creds("user", "s3cr3t");
	///
	/// Second, send the HTTP request with Lucid::Net::HTTPClientSession.
	///     Lucid::Net::HTTPClientSession session("pocoproject.org");
	///     Lucid::Net::HTTPRequest request(HTTPRequest::HTTP_GET, "/index.html", HTTPMessage::HTTP_1_1);
	///     session.sendRequest(request);
	///     Lucid::Net::HTTPResponse;
	///     std::istream& istr = session.receiveResponse(response);
	///
	/// If the server responds with a 401 status, authenticate the
	/// request and resend it:
	///     if (response.getStatus() == Lucid::Net::HTTPResponse::HTTP_UNAUTHORIZED)
	///     {
...
public:
	...
	static void extractCredentials(const Lucid::URI& uri, std::string& username, std::string& password);
