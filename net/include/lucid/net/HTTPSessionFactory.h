...
public:
	...
	HTTPSessionFactory(const std::string& proxyHost, Lucid::UInt16 proxyPort);
	...
	HTTPClientSession* createClientSession(const Lucid::URI& uri);
	...
	Lucid::UInt16 proxyPort() const;
		/// Returns the proxy port number, if one has been set, or zero otherwise.

	void setProxy(const std::string& proxyHost, Lucid::UInt16 proxyPort);
...
inline Lucid::UInt16 HTTPSessionFactory::proxyPort() const
{
	return _proxyPort;
}
