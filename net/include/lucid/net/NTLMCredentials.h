...
public:
	...
	struct BufferDesc
	{
		...
		BufferDesc(Lucid::UInt16 len, Lucid::UInt32 off):
			length(len),
			reserved(len),
			offset(off)
		{
	...
	static Lucid::UInt64 createTimestamp();
	...
	static std::vector<unsigned char> createNTLMv2Response(const std::vector<unsigned char>& ntlm2Hash, const std::vector<unsigned char>& challenge, const std::vector<unsigned char>& nonce, const std::vector<unsigned char>& targetInfo, Lucid::UInt64 timestamp);
	...
	static void readBufferDesc(Lucid::BinaryReader& reader, BufferDesc& desc);
		/// Reads a buffer descriptor.

	static void writeBufferDesc(Lucid::BinaryWriter& writer, const BufferDesc& desc);
