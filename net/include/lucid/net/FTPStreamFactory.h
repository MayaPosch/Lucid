...
class Net_API FTPStreamFactory: public Lucid::URIStreamFactory
...
public:
	...
	std::istream* open(const Lucid::URI& uri);
...
protected:
	...
	static void getUserInfo(const Lucid::URI& uri, std::string& username, std::string& password);
	static void getPathAndType(const Lucid::URI& uri, std::string& path, char& type);
