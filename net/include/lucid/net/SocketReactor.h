...
class Net_API SocketReactor: public Lucid::Runnable
...
public:
	...
	explicit SocketReactor(const Lucid::Timespan& timeout);
	...
	void addEventHandler(const Socket& socket, const Lucid::AbstractObserver& observer);
		/// Registers an event handler with the SocketReactor.
		///
		/// Usage:
		///     Lucid::Observer<MyEventHandler, SocketNotification> obs(*this, &MyEventHandler::handleMyEvent);
		///     reactor.addEventHandler(obs);

	bool hasEventHandler(const Socket& socket, const Lucid::AbstractObserver& observer);
		/// Returns true if the observer is registered with SocketReactor for the given socket.

	void removeEventHandler(const Socket& socket, const Lucid::AbstractObserver& observer);
...
private:
	typedef Lucid::AutoPtr<SocketNotifier>     NotifierPtr;
	typedef Lucid::AutoPtr<SocketNotification> NotificationPtr;
