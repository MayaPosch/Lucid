...
public:
	...
	WebSocket(const Socket& socket);
		...
		/// otherwise a Lucid::InvalidArgumentException will be thrown.
	...
	WebSocket& operator = (const Socket& socket);
		/// Assignment operator.
		///
		/// The other socket must be a WebSocket, otherwise a Lucid::InvalidArgumentException
		/// will be thrown.
	...
	void shutdown(Lucid::UInt16 statusCode, const std::string& statusMessage = "");
		/// Sends a Close control frame to the server end of
		/// the connection to initiate an orderly shutdown
		/// of the connection.
	...
	int receiveFrame(Lucid::Buffer<char>& buffer, int& flags);
		/// Receives a frame from the socket and stores it
