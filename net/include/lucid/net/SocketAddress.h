...
#include "lucid/Net/SocketAddressImpl.h"
#include <ostream>
...
public:
	...
	SocketAddress(const IPAddress& hostAddress, Lucid::UInt16 portNumber);
		/// Creates a SocketAddress from an IP address and given port number.

	explicit SocketAddress(Lucid::UInt16 port);
		/// Creates a SocketAddress with unspecified (wildcard) IP address
		/// and given port number.

	SocketAddress(Family family, Lucid::UInt16 port);
		/// Creates a SocketAddress with unspecified (wildcard) IP address
		/// of the given family, and given port number.

	SocketAddress(const std::string& hostAddress, Lucid::UInt16 portNumber);
		/// Creates a SocketAddress from an IP address and given port number.
		///
		/// The IP address must either be a domain name, or it must
		/// be in dotted decimal (IPv4) or hex string (IPv6) format.

	SocketAddress(Family family, const std::string& hostAddress, Lucid::UInt16 portNumber);
		/// Creates a SocketAddress from an IP address and given port number.
		///
		/// The IP address must either be a domain name, or it must
		/// be in dotted decimal (IPv4) or hex string (IPv6) format.
...
protected:
	void init(const IPAddress& hostAddress, Lucid::UInt16 portNumber);
	void init(const std::string& hostAddress, Lucid::UInt16 portNumber);
	void init(Family family, const std::string& hostAddress, Lucid::UInt16 portNumber);
	void init(Family family, const std::string& address);
	void init(const std::string& hostAndPort);
	Lucid::UInt16 resolveService(const std::string& service);

private:
	typedef Lucid::Net::Impl::SocketAddressImpl Impl;
	typedef Lucid::AutoPtr<Impl> Ptr;

	Ptr pImpl() const;

	void newIPv4();
	void newIPv4(const sockaddr_in*);
	void newIPv4(const IPAddress& hostAddress, Lucid::UInt16 portNumber);

#if defined(POCO_HAVE_IPv6)
	void newIPv6(const sockaddr_in6*);
	void newIPv6(const IPAddress& hostAddress, Lucid::UInt16 portNumber);
#endif
...
inline SocketAddress::Ptr SocketAddress::pImpl() const
{
	...
	throw Lucid::NullPointerException("Pointer to SocketAddress implementation is NULL.");
}


inline void SocketAddress::newIPv4()
{
	_pImpl = new Lucid::Net::Impl::IPv4SocketAddressImpl;
}


inline void SocketAddress::newIPv4(const sockaddr_in* sockAddr)
{
	_pImpl = new Lucid::Net::Impl::IPv4SocketAddressImpl(sockAddr);
}


inline void SocketAddress::newIPv4(const IPAddress& hostAddress, Lucid::UInt16 portNumber)
{
	_pImpl = new Lucid::Net::Impl::IPv4SocketAddressImpl(hostAddress.addr(), htons(portNumber));
}
...
{
	_pImpl = new Lucid::Net::Impl::IPv6SocketAddressImpl(sockAddr);
}


inline void SocketAddress::newIPv6(const IPAddress& hostAddress, Lucid::UInt16 portNumber)
{
	_pImpl = new Lucid::Net::Impl::IPv6SocketAddressImpl(hostAddress.addr(), htons(portNumber), hostAddress.scope());
}
...
{
	_pImpl = new Lucid::Net::Impl::LocalSocketAddressImpl(sockAddr);
}


inline void SocketAddress::newLocal(const std::string& path)
{
	_pImpl = new Lucid::Net::Impl::LocalSocketAddressImpl(path.c_str(), path.size());
}
...
Net_API Lucid::BinaryWriter& operator << (Lucid::BinaryWriter& writer, const Lucid::Net::SocketAddress& value);
Net_API Lucid::BinaryReader& operator >> (Lucid::BinaryReader& reader, Lucid::Net::SocketAddress& value);
Net_API std::ostream& operator << (std::ostream& ostr, const Lucid::Net::SocketAddress& address);
