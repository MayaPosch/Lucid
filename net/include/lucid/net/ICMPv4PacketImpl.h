...
public:
	...
	struct Header
	{
		Lucid::UInt8 type;          // ICMP packet type
		Lucid::UInt8 code;          // Type sub code
		Lucid::UInt16 checksum;
		Lucid::UInt16 id;
		Lucid::UInt16 seq;
	};
	...
	struct timeval time(Lucid::UInt8* buffer = 0, int length = 0) const;
	...
	bool validReplyID(Lucid::UInt8* buffer, int length) const;
		/// Returns true if the extracted id is recognized
		/// (i.e. equals the process id).
		///
		/// Buffer includes IP header, ICMP header and data.

	virtual std::string errorDescription(Lucid::UInt8* buffer, int length, int& type, int& code);
	...
	static const Lucid::UInt16 MAX_PACKET_SIZE;
	static const std::string MESSAGE_TYPE[MESSAGE_TYPE_LENGTH];
	static const Lucid::UInt8 DESTINATION_UNREACHABLE_TYPE; // 3
	static const Lucid::UInt8 SOURCE_QUENCH_TYPE;           // 4
	static const Lucid::UInt8 REDIRECT_MESSAGE_TYPE;        // 5
	static const Lucid::UInt8 TIME_EXCEEDED_TYPE;           // 11
	static const Lucid::UInt8 PARAMETER_PROBLEM_TYPE;       // 12

private:
	void initPacket();
	Header* header(Lucid::UInt8* buffer, int length) const;
	Lucid::UInt8* data(Lucid::UInt8* buffer, int length) const;
