...
{
public:
	HTTPServer(HTTPRequestHandlerFactory::Ptr pFactory, Lucid::UInt16 portNumber = 80, HTTPServerParams::Ptr pParams = new HTTPServerParams);
	...
	HTTPServer(HTTPRequestHandlerFactory::Ptr pFactory, Lucid::ThreadPool& threadPool, const ServerSocket& socket, HTTPServerParams::Ptr pParams);
		/// Creates the HTTPServer, using the given ServerSocket.
		///
		/// The server takes ownership of the HTTPRequstHandlerFactory
		/// and deletes it when it's no longer needed.
