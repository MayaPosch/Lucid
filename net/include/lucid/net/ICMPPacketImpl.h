...
public:
	...
	const Lucid::UInt8* packet(bool init = true);
	...
	virtual struct timeval time(Lucid::UInt8* buffer = 0, int length = 0) const = 0;
	...
	virtual std::string errorDescription(Lucid::UInt8* buffer, int length, int& type, int& code) = 0;
	...
	static const Lucid::UInt16 MAX_PAYLOAD_SIZE;
	static const Lucid::UInt16 MAX_SEQ_VALUE;

protected:
	Lucid::UInt16 nextSequence();
	...
	Lucid::UInt16 checksum(Lucid::UInt16 *addr, Lucid::Int32 len);
		/// Calculates the checksum for supplied buffer.

private:
	Lucid::UInt16 _seq;
	Lucid::UInt8* _pPacket;
	int _dataSize;
...
// inlines
//
inline Lucid::UInt16 ICMPPacketImpl::sequence() const
{
	return _seq;
}


inline Lucid::UInt16 ICMPPacketImpl::nextSequence()
{
	return ++_seq;
}
