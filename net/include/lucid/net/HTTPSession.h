...
public:
	...
	void setTimeout(const Lucid::Timespan& timeout);
		/// Sets the timeout for the HTTP session.
		
	void setTimeout(const Lucid::Timespan& connectionTimeout, const Lucid::Timespan& sendTimeout, const Lucid::Timespan& receiveTimeout);
		/// Sets different timeouts for the HTTP session.

	Lucid::Timespan getTimeout() const;
	...
	const Lucid::Exception* networkException() const;
	...
	void attachSessionData(const Lucid::Any& data);
	...
	const Lucid::Any& sessionData() const;
		/// Returns the data attached with attachSessionData(),
		/// or an empty Lucid::Any if no user data has been
		/// attached.
	...
	void drainBuffer(Lucid::Buffer<char>& buffer);
		/// Copies all bytes remaining in the internal buffer to the
		/// given Lucid::Buffer, resizing it as necessary.
		///
...
protected:
	...
	void setException(const Lucid::Exception& exc);
...
private:
	...
	char*            _pEnd;
	bool             _keepAlive;
	Lucid::Timespan   _connectionTimeout;
	Lucid::Timespan   _receiveTimeout;
	Lucid::Timespan   _sendTimeout;
	Lucid::Exception* _pException;
	Lucid::Any        _data;
...
inline Lucid::Timespan HTTPSession::getTimeout() const
{
...
inline const Lucid::Exception* HTTPSession::networkException() const
{
	return _pException;
}
...
inline const Lucid::Any& HTTPSession::sessionData() const
{
	return _data;
}
