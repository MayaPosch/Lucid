...
#include "lucid/Net/ParallelSocketReactor.h"
...
class ParallelSocketAcceptor
	...
	/// description see Lucid::Net::SocketAcceptor documentation.
	/// 
...
{
public:
	using ParallelReactor = Lucid::Net::ParallelSocketReactor<SR>;
	using Observer = Lucid::Observer<ParallelSocketAcceptor, ReadableNotification>;

	explicit ParallelSocketAcceptor(ServerSocket& socket,
		unsigned threads = Lucid::Environment::processorCount()):
		_socket(socket),
		_pReactor(0),
		_threads(threads),
		_next(0)
