...
public:
	...
	void connect(const SocketAddress& address, const Lucid::Timespan& timeout);
	...
	int sendBytes(Lucid::FIFOBuffer& buffer);
		/// Sends the contents of the given buffer through
	...
	int receiveBytes(Lucid::Buffer<char>& buffer, int flags = 0, const Lucid::Timespan& timeout = 100000);
		/// Receives data from the socket and stores it in buffers.
		///
		/// Returns the number of bytes received.
		///
	...
	int receiveBytes(Lucid::FIFOBuffer& buffer);
		/// Receives data from the socket and stores it
