...
#include "lucid/Net/TCPServer.h"
#include "lucid/Net/TCPServerDispatcher.h"
...
TCPServer::TCPServer(TCPServerConnectionFactory::Ptr pFactory, Lucid::UInt16 portNumber, TCPServerParams::Ptr pParams):
	_socket(ServerSocket(portNumber)),
	_thread(threadName(_socket)),
	_stopped(true)
{	
	Lucid::ThreadPool& pool = Lucid::ThreadPool::defaultPool();
	if (pParams)
	{
...
TCPServer::TCPServer(TCPServerConnectionFactory::Ptr pFactory, Lucid::ThreadPool& threadPool, const ServerSocket& socket, TCPServerParams::Ptr pParams):
	_socket(socket),
