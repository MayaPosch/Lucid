...
Lucid::UInt32 IPv4AddressImpl::scope() const
{
	return 0;
}
...
IPv6AddressImpl::IPv6AddressImpl(const void* addr, Lucid::UInt32 scope): _scope(scope)
{
	std::memcpy(&_addr, addr, sizeof(_addr));
}
	...
	_scope(0)
{
	...
	if (prefix > 0)
	{
		_addr.s6_addr16[i++] = ByteOrder::toNetwork(static_cast<Lucid::UInt16>(~(0xffff >> prefix)));
	}
	while (i < 8)
	{
		_addr.s6_addr16[i++] = 0;
...
Lucid::UInt32 IPv6AddressImpl::scope() const
{
	return _scope;
}
...
{
	throw Lucid::NotImplementedException("mask() is only supported for IPv4 addresses");
}
...
IPv6AddressImpl IPv6AddressImpl::operator & (const IPv6AddressImpl& addr) const
{
	if (_scope != addr._scope)
		throw Lucid::InvalidArgumentException("Scope ID of passed IPv6 address does not match with the source one.");
