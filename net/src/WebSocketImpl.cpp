...
#define NOMINMAX
#include "lucid/Net/WebSocketImpl.h"
#include "lucid/Net/NetException.h"
#include "lucid/Net/WebSocket.h"
...
{
	Lucid::Buffer<char> frame(length + MAX_HEADER_LENGTH);
	Lucid::MemoryOutputStream ostr(frame.begin(), frame.size());
	Lucid::BinaryWriter writer(ostr, Lucid::BinaryWriter::NETWORK_BYTE_ORDER);

	if (flags == 0) flags = WebSocket::FRAME_BINARY;
	flags &= 0xff;
	writer << static_cast<Lucid::UInt8>(flags);
	Lucid::UInt8 lengthByte(0);
	if (_mustMaskPayload)
	{
	...
	if (length < 126)
	{
		lengthByte |= static_cast<Lucid::UInt8>(length);
		writer << lengthByte;
	}
	else if (length < 65536)
	{
		lengthByte |= 126;
		writer << lengthByte << static_cast<Lucid::UInt16>(length);
	}
	else
	{
		lengthByte |= 127;
		writer << lengthByte << static_cast<Lucid::UInt64>(length);
	}
	if (_mustMaskPayload)
	{
		const Lucid::UInt32 mask = _rnd.next();
...
int WebSocketImpl::receiveHeader(char mask[4], bool& useMask)
{
	...
	poco_assert (n == 2);
	Lucid::UInt8 flags = static_cast<Lucid::UInt8>(header[0]);
	_frameFlags = flags;
	Lucid::UInt8 lengthByte = static_cast<Lucid::UInt8>(header[1]);
	...
	if (lengthByte == 127)
	{
		...
		Lucid::MemoryInputStream istr(header + 2, 8);
		Lucid::BinaryReader reader(istr, Lucid::BinaryReader::NETWORK_BYTE_ORDER);
		Lucid::UInt64 l;
		reader >> l;
	...
	else if (lengthByte == 126)
	{
		...
		Lucid::MemoryInputStream istr(header + 2, 2);
		Lucid::BinaryReader reader(istr, Lucid::BinaryReader::NETWORK_BYTE_ORDER);
		Lucid::UInt16 l;
		reader >> l;
		if (l > _maxPayloadSize) throw WebSocketException("Payload too big", WebSocket::WS_ERR_PAYLOAD_TOO_BIG);
...
int WebSocketImpl::receiveBytes(void* buffer, int length, int)
{
	...
	if (payloadLength > length)
		throw WebSocketException(Lucid::format("Insufficient buffer for payload size %d", payloadLength), WebSocket::WS_ERR_PAYLOAD_TOO_BIG);
	return receivePayload(reinterpret_cast<char*>(buffer), payloadLength, mask, useMask);
}


int WebSocketImpl::receiveBytes(Lucid::Buffer<char>& buffer, int, const Lucid::Timespan&)
{
	char mask[4];
	bool useMask;
	int payloadLength = receiveHeader(mask, useMask);
...
{
	throw Lucid::InvalidAccessException("Cannot acceptConnection() on a WebSocketImpl");
}


void WebSocketImpl::connect(const SocketAddress& address)
{
	throw Lucid::InvalidAccessException("Cannot connect() a WebSocketImpl");
}


void WebSocketImpl::connect(const SocketAddress& address, const Lucid::Timespan& timeout)
{
	throw Lucid::InvalidAccessException("Cannot connect() a WebSocketImpl");
}
...
{
	throw Lucid::InvalidAccessException("Cannot connectNB() a WebSocketImpl");
}


void WebSocketImpl::bind(const SocketAddress& address, bool reuseAddress)
{
	throw Lucid::InvalidAccessException("Cannot bind() a WebSocketImpl");
}
...
{
	throw Lucid::InvalidAccessException("Cannot bind6() a WebSocketImpl");
}
...
{
	throw Lucid::InvalidAccessException("Cannot listen() on a WebSocketImpl");
}
...
{
	throw Lucid::InvalidAccessException("Cannot sendTo() on a WebSocketImpl");
}


int WebSocketImpl::receiveFrom(void* buffer, int length, SocketAddress& address, int flags)
{
	throw Lucid::InvalidAccessException("Cannot receiveFrom() on a WebSocketImpl");
}


void WebSocketImpl::sendUrgent(unsigned char data)
{
	throw Lucid::InvalidAccessException("Cannot sendUrgent() on a WebSocketImpl");
}
...
void WebSocketImpl::setSendTimeout(const Lucid::Timespan& timeout)
{
	_pStreamSocketImpl->setSendTimeout(timeout);
}


Lucid::Timespan WebSocketImpl::getSendTimeout()
{
	return _pStreamSocketImpl->getSendTimeout();
}


void WebSocketImpl::setReceiveTimeout(const Lucid::Timespan& timeout)
{
	_pStreamSocketImpl->setReceiveTimeout(timeout);
}


Lucid::Timespan WebSocketImpl::getReceiveTimeout()
{
