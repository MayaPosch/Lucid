...
#include "lucid/Net/SMTPChannel.h"
...
void SMTPChannel::log(const Message& msg)
{
	try
	{
		...
		{
			{
				Lucid::FileInputStream fis(_attachment, std::ios::in | std::ios::binary | std::ios::ate);
				if (fis.good())
				{
...
{
	Lucid::LoggingFactory::defaultFactory().registerChannelClass("SMTPChannel", 
		new Lucid::Instantiator<SMTPChannel, Lucid::Channel>);
}
