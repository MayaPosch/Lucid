...
{
	return icompare(header, 0, 5, "Basic") == 0 && (header.size() > 5 ? Lucid::Ascii::isSpace(header[5]) : true);
}


bool HTTPCredentials::isDigestCredentials(const std::string& header)
{
	return icompare(header, 0, 6, "Digest") == 0 && (header.size() > 6 ? Lucid::Ascii::isSpace(header[6]) : true);
}


bool HTTPCredentials::isNTLMCredentials(const std::string& header)
{
	return icompare(header, 0, 4, "NTLM") == 0 && (header.size() > 5 ? Lucid::Ascii::isSpace(header[5]) : true);
}
...
void HTTPCredentials::extractCredentials(const Lucid::URI& uri, std::string& username, std::string& password)
{
