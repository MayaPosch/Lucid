...
void MessageHeader::read(std::istream& istr)
{
	...
	while (ch != eof && ch != '\r' && ch != '\n')
	{
		...
		while (ch != eof && Lucid::Ascii::isSpace(ch) && ch != '\r' && ch != '\n') ch = buf.sbumpc();
...
void MessageHeader::quote(const std::string& value, std::string& result, bool allowSpace)
{
	...
	{
		if (!Lucid::Ascii::isAlphaNumeric(*it) && *it != '.' && *it != '_' && *it != '-' && !(Lucid::Ascii::isSpace(*it) && allowSpace))
			mustQuote = true;
	}
	if (mustQuote) result += '"';
	result.append(value);
