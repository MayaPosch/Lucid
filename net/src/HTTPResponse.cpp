...
void HTTPResponse::setDate(const Lucid::Timestamp& dateTime)
{
	set(DATE, DateTimeFormatter::format(dateTime, DateTimeFormat::HTTP_FORMAT));
}


Lucid::Timestamp HTTPResponse::getDate() const
{
...
void HTTPResponse::getCookies(std::vector<HTTPCookie>& cookies) const
{
	...
	while (it != end() && Lucid::icompare(it->first, SET_COOKIE) == 0)
	{
...
void HTTPResponse::read(std::istream& istr)
{
	...
	while (Lucid::Ascii::isSpace(ch)) ch = istr.get();
	while (!Lucid::Ascii::isSpace(ch) && ch != eof && status.length() < MAX_STATUS_LENGTH) { status += (char) ch; ch = istr.get(); }
	if (!Lucid::Ascii::isSpace(ch)) throw MessageException("Invalid HTTP status code");
	while (Lucid::Ascii::isSpace(ch) && ch != '\r' && ch != '\n' && ch != eof) ch = istr.get();
	while (ch != '\r' && ch != '\n' && ch != eof && reason.length() < MAX_REASON_LENGTH) { reason += (char) ch; ch = istr.get(); }
	if (!Lucid::Ascii::isSpace(ch)) throw MessageException("HTTP reason string too long");
	if (ch == '\r') ch = istr.get();
