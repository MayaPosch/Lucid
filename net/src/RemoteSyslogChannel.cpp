...
void RemoteSyslogChannel::log(const Message& msg)
{
	...
	m.reserve(1024);
	m += '<';
	Lucid::NumberFormatter::append(m, getPrio(msg) + _facility);
	m += '>';
	if (_bsdFormat)
	{
		Lucid::DateTimeFormatter::append(m, msg.getTime(), BSD_TIMEFORMAT);
		m += ' ';
		m += _host;
	}
	else
	{
		m += "1 "; // version
		Lucid::DateTimeFormatter::append(m, msg.getTime(), SYSLOG_TIMEFORMAT);
		m += ' ';
		m += _host;
		m += ' ';
		m += _name;
		m += ' ';
		Lucid::NumberFormatter::append(m, msg.getPid());
		m += ' ';
		m += msg.getSource();
		m += ' ';
...
void RemoteSyslogChannel::setProperty(const std::string& name, const std::string& value)
{
	...
	{
		std::string facility;
		if (Lucid::icompare(value, 4, "LOG_") == 0)
			facility = Lucid::toUpper(value.substr(4));
		else if (Lucid::icompare(value, 4, "SYSLOG_") == 0)
			facility = Lucid::toUpper(value.substr(7));
		else
			facility = Lucid::toUpper(value);
...
{
	Lucid::LoggingFactory::defaultFactory().registerChannelClass("RemoteSyslogChannel", new Lucid::Instantiator<RemoteSyslogChannel, Lucid::Channel>);
}
