...
#include "lucid/Net/SMTPClientSession.h"
#include "lucid/Net/MailMessage.h"
#include "lucid/Net/MailRecipient.h"
#include "lucid/Net/MailStream.h"
#include "lucid/Net/SocketAddress.h"
#include "lucid/Net/SocketStream.h"
#include "lucid/Net/NetException.h"
#include "lucid/Net/NetworkInterface.h"
...
using Lucid::DigestEngine;
using Lucid::HMACEngine;
using Lucid::MD5Engine;
using Lucid::SHA1Engine;
using Lucid::DigestOutputStream;
...
SMTPClientSession::SMTPClientSession(const std::string& host, Lucid::UInt16 port):
	_host(host),
...
void SMTPClientSession::setTimeout(const Lucid::Timespan& timeout)
{
	_socket.setReceiveTimeout(timeout);
}


Lucid::Timespan SMTPClientSession::getTimeout() const
{
...
void SMTPClientSession::loginUsingCRAM(const std::string& username, const std::string& method, Lucid::DigestEngine& hmac)
{
	std::string response;
...
void SMTPClientSession::loginUsingLogin(const std::string& username, const std::string& password)
{
	...
	if (Lucid::icompare(decodedResponse, 0, 8, "username") == 0) // username first (md5("Username:"))
	{
	...
	else if  (Lucid::icompare(decodedResponse, 0, 8, "password") == 0) // password first (md5("Password:"))
	{
		status = sendCommand(passwordBase64.str(), response);
...
void SMTPClientSession::loginUsingNTLM(const std::string& username, const std::string& password)
{
	...
	Lucid::SharedPtr<NTLMContext> pNTLMContext;
	...
	if (status == 334)
	{
		...
		else
		{
			...
			if (NTLMCredentials::parseChallengeMessage(&buffer[0], buffer.size(), challengeMsg))
			{
				...
				Lucid::UInt64 timestamp = NTLMCredentials::createTimestamp();
