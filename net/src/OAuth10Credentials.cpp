...
OAuth10Credentials::OAuth10Credentials(const Lucid::Net::HTTPRequest& request)
{
...
void OAuth10Credentials::authenticate(HTTPRequest& request, const Lucid::URI& uri, SignatureMethod method)
{
...
void OAuth10Credentials::authenticate(HTTPRequest& request, const Lucid::URI& uri, const Lucid::Net::HTMLForm& params, SignatureMethod method)
{
...
bool OAuth10Credentials::verify(const HTTPRequest& request, const Lucid::URI& uri)
{
	HTMLForm params;
...
bool OAuth10Credentials::verify(const HTTPRequest& request, const Lucid::URI& uri, const Lucid::Net::HTMLForm& params)
{
	if (request.hasCredentials())
	{
		std::string authScheme;
...
void OAuth10Credentials::signPlaintext(Lucid::Net::HTTPRequest& request) const
{
	...
	if (!_realm.empty())
	{
		Lucid::format(authorization, " realm=\"%s\",", _realm);
	}
	Lucid::format(authorization, " oauth_consumer_key=\"%s\"", percentEncode(_consumerKey));
	Lucid::format(authorization, ", oauth_signature=\"%s\"", percentEncode(signature));
	authorization += ", oauth_signature_method=\"PLAINTEXT\"";
	if (!_token.empty())
	{
		Lucid::format(authorization, ", oauth_token=\"%s\"", percentEncode(_token));
	}
	if (!_callback.empty())
	{
		Lucid::format(authorization, ", oauth_callback=\"%s\"", percentEncode(_callback));
	}
...
void OAuth10Credentials::signHMACSHA1(Lucid::Net::HTTPRequest& request, const std::string& uri, const Lucid::Net::HTMLForm& params) const
{
	...
	if (timestamp.empty())
	{
		timestamp = Lucid::NumberFormatter::format(Lucid::Timestamp().epochTime());
	}
	...
	Lucid::format(authorization, " oauth_consumer_key=\"%s\"", percentEncode(_consumerKey));
	Lucid::format(authorization, ", oauth_nonce=\"%s\"", percentEncode(nonce));
	Lucid::format(authorization, ", oauth_signature=\"%s\"", percentEncode(signature));
	authorization += ", oauth_signature_method=\"HMAC-SHA1\"";
	Lucid::format(authorization, ", oauth_timestamp=\"%s\"", timestamp);
	if (!_token.empty())
	{
		Lucid::format(authorization, ", oauth_token=\"%s\"", percentEncode(_token));
	}
...
std::string OAuth10Credentials::createNonce() const
{
	...
	Lucid::Base64Encoder base64Encoder(base64Nonce);
	Lucid::RandomInputStream randomStream;
	for (int i = 0; i < 32; i++)
	{
	...
	return Lucid::translate(nonce, "+/=", "");
}


std::string OAuth10Credentials::createSignature(const Lucid::Net::HTTPRequest& request, const std::string& uri, const Lucid::Net::HTMLForm& params, const std::string& nonce, const std::string& timestamp) const
{
	...
	Lucid::HMACEngine<Lucid::SHA1Engine> hmacEngine(signingKey);
	hmacEngine.update(signatureBase);
	Lucid::DigestEngine::Digest digest = hmacEngine.digest();
	std::ostringstream digestBase64;
	Lucid::Base64Encoder base64Encoder(digestBase64);
