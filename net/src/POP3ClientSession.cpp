...
#include "lucid/Net/POP3ClientSession.h"
...
class DialogStreamBuf: public Lucid::UnbufferedStreamBuf
{
public:
...
POP3ClientSession::POP3ClientSession(const std::string& host, Lucid::UInt16 port):	
...
void POP3ClientSession::setTimeout(const Lucid::Timespan& timeout)
{
	_socket.setReceiveTimeout(timeout);
}

	
Lucid::Timespan POP3ClientSession::getTimeout() const
{
...
int POP3ClientSession::messageCount()
{
	...
	while (it != end && Lucid::Ascii::isSpace(*it)) ++it;
	while (it != end && Lucid::Ascii::isDigit(*it)) count = count*10 + *it++ - '0';
	return count;
}
...
void POP3ClientSession::listMessages(MessageInfoVec& messages)
{
	...
	while (response != ".")
	{
		...
		std::string::const_iterator it  = response.begin();
		std::string::const_iterator end = response.end();
		while (it != end && Lucid::Ascii::isDigit(*it)) info.id = info.id*10 + *it++ - '0';
		while (it != end && Lucid::Ascii::isSpace(*it)) ++it;
		while (it != end && Lucid::Ascii::isDigit(*it)) info.size = info.size*10 + *it++ - '0';
