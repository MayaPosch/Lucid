...
using Lucid::IllegalStateException;
...
HTTPClientSession::HTTPClientSession(const std::string& host, Lucid::UInt16 port):
	_host(host),
	_port(port),
	_proxyConfig(_globalProxyConfig),
	_keepAliveTimeout(DEFAULT_KEEP_ALIVE_TIMEOUT, 0),
...
HTTPClientSession::HTTPClientSession(const std::string& host, Lucid::UInt16 port, const ProxyConfig& proxyConfig):
	_host(host),
	_port(port),
...
void HTTPClientSession::setPort(Lucid::UInt16 port)
{
	if (!connected())
		_port = port;
	else
...
void HTTPClientSession::setProxy(const std::string& host, Lucid::UInt16 port)
{
	if (!connected())
	{
...
void HTTPClientSession::setProxyPort(Lucid::UInt16 port)
{
	if (!connected())
		_proxyConfig.port = port;
	else
...
void HTTPClientSession::setKeepAliveTimeout(const Lucid::Timespan& timeout)
{
