...
void WebSocket::shutdown(Lucid::UInt16 statusCode, const std::string& statusMessage)
{
	Lucid::Buffer<char> buffer(statusMessage.size() + 2);
	Lucid::MemoryOutputStream ostr(buffer.begin(), buffer.size());
...
int WebSocket::receiveFrame(Lucid::Buffer<char>& buffer, int& flags)
{
...
WebSocketImpl* WebSocket::connect(HTTPClientSession& cs, HTTPRequest& request, HTTPResponse& response, HTTPCredentials& credentials)
{
	...
	else if (response.getStatus() == HTTPResponse::HTTP_UNAUTHORIZED)
	{
		if (!credentials.empty())
		{
			...
			Lucid::StreamCopier::copyStream(istr, null);
...
WebSocketImpl* WebSocket::completeHandshake(HTTPClientSession& cs, HTTPResponse& response, const std::string& key)
{
	...
	if (Lucid::icompare(connection, "Upgrade") != 0)
	...
	if (Lucid::icompare(upgrade, "websocket") != 0)
...
std::string WebSocket::createKey()
{
	...
	Lucid::BinaryWriter writer(base64);
...
std::string WebSocket::computeAccept(const std::string& key)
{
	...
	Lucid::SHA1Engine sha1;
	sha1.update(accept);
	Lucid::DigestEngine::Digest d = sha1.digest();
	std::ostringstream ostr;
	Lucid::Base64Encoder base64(ostr);
