...
using Lucid::Net::Impl::SocketAddressImpl;
using Lucid::Net::Impl::IPv4SocketAddressImpl;
#ifdef POCO_HAVE_IPv6
using Lucid::Net::Impl::IPv6SocketAddressImpl;
#endif
#ifdef POCO_OS_FAMILY_UNIX
using Lucid::Net::Impl::LocalSocketAddressImpl;
#endif
...
SocketAddress::SocketAddress(const IPAddress& hostAddress, Lucid::UInt16 portNumber)
{
	init(hostAddress, portNumber);
}


SocketAddress::SocketAddress(Lucid::UInt16 portNumber)
{
	init(IPAddress(), portNumber);
}


SocketAddress::SocketAddress(Family fam, Lucid::UInt16 portNumber)
{
	init(IPAddress(fam), portNumber);
}


SocketAddress::SocketAddress(const std::string& hostAddress, Lucid::UInt16 portNumber)
{
	init(hostAddress, portNumber);
}


SocketAddress::SocketAddress(Family fam, const std::string& hostAddress, Lucid::UInt16 portNumber)
{
...
#endif
	else throw Lucid::InvalidArgumentException("Invalid address length or family passed to SocketAddress()");
}
...
Lucid::UInt16 SocketAddress::port() const
{
...
void SocketAddress::init(const IPAddress& hostAddress, Lucid::UInt16 portNumber)
{
...
#endif
	else throw Lucid::NotImplementedException("unsupported IP address family");
}


void SocketAddress::init(const std::string& hostAddress, Lucid::UInt16 portNumber)
{
	IPAddress ip;
...
void SocketAddress::init(Family fam, const std::string& hostAddress, Lucid::UInt16 portNumber)
{
	IPAddress ip;
	if (IPAddress::tryParse(hostAddress, ip))
	{
...
Lucid::UInt16 SocketAddress::resolveService(const std::string& service)
{
	unsigned port;
...
Lucid::BinaryWriter& operator << (Lucid::BinaryWriter& writer, const Lucid::Net::SocketAddress& value)
{
	writer << value.host();
	writer << value.port();
	return writer;
...
Lucid::BinaryReader& operator >> (Lucid::BinaryReader& reader, Lucid::Net::SocketAddress& value)
{
	Lucid::Net::IPAddress host;
	reader >> host;
	Lucid::UInt16 port;
	reader >> port;
	value = Lucid::Net::SocketAddress(host, port);
	return reader;
}


std::ostream& operator << (std::ostream& ostr, const Lucid::Net::SocketAddress& address)
{
