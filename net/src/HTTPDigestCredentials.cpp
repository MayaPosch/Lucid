...
namespace
{
	std::string digest(Lucid::DigestEngine& engine,
					   const std::string& a,
					   const std::string& b,
					   ...
					   const std::string& f = std::string())
	{
		...
		return Lucid::DigestEngine::digestToHex(engine.digest());
	}

	std::string formatNonceCounter(int counter)
	{
		return Lucid::NumberFormatter::formatHex(counter, 8);
	}
}
...
Lucid::FastMutex HTTPDigestCredentials::_nonceMutex;
...
{
	Lucid::FastMutex::ScopedLock lock(_nonceMutex);
...
void HTTPDigestCredentials::createAuthParams(const HTTPRequest& request, const HTTPAuthenticationParams& responseAuthParams)
{
	...
	{
		Lucid::StringTokenizer tok(qop, ",", Lucid::StringTokenizer::TOK_TRIM);
