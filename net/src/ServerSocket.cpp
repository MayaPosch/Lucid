...
#include "lucid/Net/ServerSocketImpl.h"
...
ServerSocket::ServerSocket(Lucid::UInt16 port, int backlog): Socket(new ServerSocketImpl)
{
...
void ServerSocket::bind(Lucid::UInt16 port, bool reuseAddress)
{
	IPAddress wildcardAddr;
	SocketAddress address(wildcardAddr, port);
	impl()->bind(address, reuseAddress);
...
void ServerSocket::bind(Lucid::UInt16 port, bool reuseAddress, bool reusePort)
{
	IPAddress wildcardAddr;
	SocketAddress address(wildcardAddr, port);
	impl()->bind(address, reuseAddress, reusePort);
...
void ServerSocket::bind6(Lucid::UInt16 port, bool reuseAddress, bool ipV6Only)
{
...
void ServerSocket::bind6(Lucid::UInt16 port, bool reuseAddress, bool reusePort, bool ipV6Only)
{
#if defined(POCO_HAVE_IPv6)
	IPAddress wildcardAddr(IPAddress::IPv6);
	SocketAddress address(wildcardAddr, port);
