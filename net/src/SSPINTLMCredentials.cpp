...
#include "lucid/Net/SSPINTLMCredentials.h"


#if POCO_OS == POCO_OS_WINDOWS_NT


#include "lucid/SharedLibrary.h"
...
public:
		...
		_pSecFunTable(0)
	{
		...
		if (!_pSecFunTable) throw Lucid::SystemException("Failed to initialize SSPI");
	}
	...
	Lucid::SharedPtr<NTLMContext> createNTLMContext(const std::string& host, const std::string& service)
	{
		PSecPkgInfoW pSecPkgInfo;
		SECURITY_STATUS status = _pSecFunTable->QuerySecurityPackageInfoW(L"NTLM", &pSecPkgInfo);
		if (status != SEC_E_OK) throw Lucid::SystemException("NTLM SSPI not available", status);

		std::size_t maxTokenSize = pSecPkgInfo->cbMaxToken;
		_pSecFunTable->FreeContextBuffer(pSecPkgInfo);

		Lucid::SharedPtr<NTLMContext> pContext = new NTLMContext(new NTLMContextImpl);
		...
		if (status != SEC_E_OK) throw Lucid::SystemException("Failed to acquire NTLM credentials", status);

		std::string spn = service;
		spn += '/';
		spn += host;
		Lucid::UnicodeConverter::convert(spn, pContext->_pImpl->spn);
	...
	std::vector<unsigned char> negotiate(NTLMContext& context)
	{
		...
		{
			throw Lucid::SystemException("Failed to initialize NTLM security context", status);
		}
	...
	std::vector<unsigned char> authenticate(NTLMContext& context, const std::vector<unsigned char>& challenge)
	{
		...
		if (status != SEC_E_OK)
		{
			throw Lucid::SystemException("Failed to create NTLM authenticate message", status);
		}
...
private:
	...
	Lucid::SharedLibrary _securityLib;
...
namespace
{
	static Lucid::SingletonHolder<SSPINTLMProvider> sspintlmProviderHolder;
}
...
Lucid::SharedPtr<NTLMContext> SSPINTLMCredentials::createNTLMContext(const std::string& workstation, const std::string& service)
{
#if POCO_OS == POCO_OS_WINDOWS_NT
	return SSPINTLMProvider::instance().createNTLMContext(workstation, service);
#else
	throw Lucid::NotImplementedException("SSPINTLMCredentials::createNTLMContext() is only available on Windows");
#endif
}
...
#else
	throw Lucid::NotImplementedException("SSPINTLMCredentials::negotiate() is only available on Windows");
#endif
}
...
#else
	throw Lucid::NotImplementedException("SSPINTLMCredentials::authenticate() is only available on Windows");
#endif
}
