...
#include "lucid/Net/FTPClientSession.h"
...
void FTPClientSession::setTimeout(const Lucid::Timespan& timeout)
{
	if (!isOpen())
...
Lucid::Timespan FTPClientSession::getTimeout() const
{
	return _timeout;
}
...
void  FTPClientSession::receiveServerReadyReply()
{
	...
	{
		Lucid::FastMutex::ScopedLock lock(_wmMutex);
...
void FTPClientSession::parseAddress(const std::string& str, SocketAddress& addr)
{
	...
	if (it != end) ++it;
	std::string host;
	while (it != end && Lucid::Ascii::isDigit(*it)) host += *it++;
	...
	while (it != end && Lucid::Ascii::isDigit(*it)) host += *it++;
	if (it != end && *it == ',') ++it;
	Lucid::UInt16 portHi = 0;
	while (it != end && Lucid::Ascii::isDigit(*it)) { portHi *= 10; portHi += *it++ - '0'; }
	if (it != end && *it == ',') ++it;
	Lucid::UInt16 portLo = 0;
	while (it != end && Lucid::Ascii::isDigit(*it)) { portLo *= 10; portLo += *it++ - '0'; }
...
void FTPClientSession::parseExtAddress(const std::string& str, SocketAddress& addr)
{
	...
	if (it != end && *it == delim) ++it;
	Lucid::UInt16 port = 0;
	while (it != end && Lucid::Ascii::isDigit(*it)) { port *= 10; port += *it++ - '0'; }
