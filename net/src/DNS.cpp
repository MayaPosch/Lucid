...
#include "lucid/RWLock.h"
#include "lucid/TextIterator.h"
#include "lucid/TextConverter.h"
#include "lucid/UTF8Encoding.h"
#include "lucid/UTF32Encoding.h"
#include "lucid/Unicode.h"
#include <cstring>
...
typedef Lucid::UInt32 punycode_uint;
...
#if defined(POCO_HAVE_LIBRESOLV)
static Lucid::RWLock resolverLock;
#endif
...
{
#if defined(POCO_HAVE_LIBRESOLV)
	Lucid::ScopedReadRWLock readLock(resolverLock);
#endif
...
{
#if defined(POCO_HAVE_LIBRESOLV)
	Lucid::ScopedWriteRWLock writeLock(resolverLock);
	res_init();
#endif
}
...
std::string DNS::encodeIDNLabel(const std::string& label)
{
	...
	std::vector<Lucid::UInt32> uniLabel;
	Lucid::UTF8Encoding utf8;
	Lucid::TextIterator it(label, utf8);
	Lucid::TextIterator end(label);
	while (it != end)
	{
		int ch = *it;
		if (ch < 0) throw DNSException("Invalid UTF-8 character in IDN label", label);
		if (Lucid::Unicode::isUpper(ch))
		{
			ch = Lucid::Unicode::toLower(ch);
		}
		uniLabel.push_back(static_cast<Lucid::UInt32>(ch));
		++it;
	}
	char buffer[64];
	std::size_t size = 64;
...
std::string DNS::decodeIDNLabel(const std::string& encodedIDN)
{
	...
	if (encodedIDN.compare(0, 4, "xn--") == 0)
	{
		...
		if (rc == punycode_success)
		{
			Lucid::UTF32Encoding utf32;
			Lucid::UTF8Encoding utf8;
			Lucid::TextConverter converter(utf32, utf8);
