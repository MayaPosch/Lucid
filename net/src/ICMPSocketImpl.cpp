...
#include "lucid/Net/ICMPSocketImpl.h"
...
int ICMPSocketImpl::receiveFrom(void*, int, SocketAddress& address, int flags)
{
	...
	Lucid::Buffer<unsigned char> buffer(maxPacketSize);
	...
	if (expected > 0)
	{
		throw ICMPException(Lucid::format("No response: expected %d, received: %d", _icmpPacket.packetSize(),
