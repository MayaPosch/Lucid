...
#include "lucid/Net/SocketNotifier.h"
...
SocketReactor::SocketReactor(const Lucid::Timespan& timeout):
	_stop(false),
	_timeout(timeout),
	_pReadableNotification(new ReadableNotification(this)),
	_pWritableNotification(new WritableNotification(this)),
...
void SocketReactor::setTimeout(const Lucid::Timespan& timeout)
{
	_timeout = timeout;
}


const Lucid::Timespan& SocketReactor::getTimeout() const
{
	return _timeout;
}


void SocketReactor::addEventHandler(const Socket& socket, const Lucid::AbstractObserver& observer)
{
...
bool SocketReactor::hasEventHandler(const Socket& socket, const Lucid::AbstractObserver& observer)
{
...
void SocketReactor::removeEventHandler(const Socket& socket, const Lucid::AbstractObserver& observer)
{
	NotifierPtr pNotifier = getNotifier(socket);
