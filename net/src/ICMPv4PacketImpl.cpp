...
const Lucid::UInt8 ICMPv4PacketImpl::SOURCE_QUENCH_TYPE     = 4;
const Lucid::UInt8 ICMPv4PacketImpl::REDIRECT_MESSAGE_TYPE  = 5;
const UInt8 ICMPv4PacketImpl::TIME_EXCEEDED_TYPE           = 11;
const Lucid::UInt8 ICMPv4PacketImpl::PARAMETER_PROBLEM_TYPE = 12;
...
void ICMPv4PacketImpl::initPacket()
{
	...
	icp->id       = 0;
#else
	icp->id       = static_cast<UInt16>(Lucid::Process::id());
#endif
...
struct timeval ICMPv4PacketImpl::time(Lucid::UInt8* buffer, int length) const
{
	struct timeval tv;
...
ICMPv4PacketImpl::Header* ICMPv4PacketImpl::header(Lucid::UInt8* buffer, int length) const
{
...
Lucid::UInt8* ICMPv4PacketImpl::data(Lucid::UInt8* buffer, int length) const
{
	return ((Lucid::UInt8*) header(buffer, length)) + sizeof(Header);
}


bool ICMPv4PacketImpl::validReplyID(Lucid::UInt8* buffer, int length) const
{
	Header *icp = header(buffer, length);
#if defined(POCO_VXWORKS)
	return icp && icp->id == 0;
#else
	return icp && (static_cast<Lucid::UInt16>(Process::id()) == icp->id);
#endif
}
