...
void HTTPRequest::setHost(const std::string& host, Lucid::UInt16 port)
{
	std::string value;
...
void HTTPRequest::getCookies(NameValueCollection& cookies) const
{
	...
	while (it != end() && Lucid::icompare(it->first, COOKIE) == 0)
	{
...
void HTTPRequest::read(std::istream& istr)
{
	...
	while (Lucid::Ascii::isSpace(ch)) ch = istr.get();
	if (ch == eof) throw MessageException("No HTTP request header");
	while (!Lucid::Ascii::isSpace(ch) && ch != eof && method.length() < MAX_METHOD_LENGTH) { method += (char) ch; ch = istr.get(); }
	if (!Lucid::Ascii::isSpace(ch)) throw MessageException("HTTP request method invalid or too long");
	while (Lucid::Ascii::isSpace(ch)) ch = istr.get();
	while (!Lucid::Ascii::isSpace(ch) && ch != eof && uri.length() < MAX_URI_LENGTH) { uri += (char) ch; ch = istr.get(); }
	if (!Lucid::Ascii::isSpace(ch)) throw MessageException("HTTP request URI invalid or too long");
	while (Lucid::Ascii::isSpace(ch)) ch = istr.get();
	while (!Lucid::Ascii::isSpace(ch) && ch != eof && version.length() < MAX_VERSION_LENGTH) { version += (char) ch; ch = istr.get(); }
	if (!Lucid::Ascii::isSpace(ch)) throw MessageException("Invalid HTTP version string");
	while (ch != '\n' && ch != eof) { ch = istr.get(); }
...
void HTTPRequest::getCredentials(const std::string& header, std::string& scheme, std::string& authInfo) const
{
	...
	if (has(header))
	{
		...
		while (it != end && Lucid::Ascii::isSpace(*it)) ++it;
		while (it != end && !Lucid::Ascii::isSpace(*it)) scheme += *it++;
		while (it != end && Lucid::Ascii::isSpace(*it)) ++it;
