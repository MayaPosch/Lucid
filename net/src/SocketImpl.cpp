...
#include "lucid/Net/StreamSocketImpl.h"
...
void SocketImpl::connect(const SocketAddress& address, const Lucid::Timespan& timeout)
{
	if (_sockfd == POCO_INVALID_SOCKET)
	{
		init(address.af());
	...
	try
	{
		...
		if (rc != 0)
		{
			...
			if (!poll(timeout, SELECT_READ | SELECT_WRITE | SELECT_ERROR))
				throw Lucid::TimeoutException("connect timed out", address.toString());
			err = socketError();
			if (err != 0) error(err);
		}
	}
...
void SocketImpl::bind6(const SocketAddress& address, bool reuseAddress, bool reusePort, bool ipV6Only)
{
	...
	if (address.family() != SocketAddress::IPv6)
		throw Lucid::InvalidArgumentException("SocketAddress must be an IPv6 address");
...
#else
	if (ipV6Only) throw Lucid::NotImplementedException("IPV6_V6ONLY not defined.");
#endif
	if (reuseAddress)
		setReuseAddress(true);
	if (reusePort)
	...
	if (rc != 0) error(address.toString());
#else
	throw Lucid::NotImplementedException("No IPv6 support available");
#endif
}
...
void SocketImpl::checkBrokenTimeout(SelectMode mode)
{
	if (_isBrokenTimeout)
	{
		Lucid::Timespan timeout = (mode == SELECT_READ) ? _recvTimeout : _sndTimeout;
...
int SocketImpl::receiveBytes(Lucid::Buffer<char>& buffer, int flags, const Lucid::Timespan& timeout)
{
	int rc = 0;
...
bool SocketImpl::poll(const Lucid::Timespan& timeout, int mode)
{
	...
	Lucid::Timespan remainingTime(timeout);
	int rc;
	do
	{
...
void SocketImpl::setSendTimeout(const Lucid::Timespan& timeout)
{
...
Lucid::Timespan SocketImpl::getSendTimeout()
{
	Timespan result;
#if defined(_WIN32) && !defined(POCO_BROKEN_TIMEOUTS)
	int value;
...
void SocketImpl::setReceiveTimeout(const Lucid::Timespan& timeout)
{
...
Lucid::Timespan SocketImpl::getReceiveTimeout()
{
	Timespan result;
#if defined(_WIN32) && !defined(POCO_BROKEN_TIMEOUTS)
	int value;
...
void SocketImpl::setOption(int level, int option, const Lucid::Timespan& value)
{
	struct timeval tv;
...
void SocketImpl::getOption(int level, int option, Lucid::Timespan& value)
{
	struct timeval tv;
