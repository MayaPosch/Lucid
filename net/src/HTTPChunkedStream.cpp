...
int HTTPChunkedStreamBuf::readFromDevice(char* buffer, std::streamsize length)
{
	...
	{
		int ch = _session.get();
		while (Lucid::Ascii::isSpace(ch)) ch = _session.get();
		std::string chunkLen;
		while (Lucid::Ascii::isHexDigit(ch) && chunkLen.size() < 8) { chunkLen += (char) ch; ch = _session.get(); }
		if (ch != eof && !(Lucid::Ascii::isSpace(ch) || ch == ';')) return eof;
		while (ch != eof && ch != '\n') ch = _session.get();
		unsigned chunk;
...
Lucid::MemoryPool HTTPChunkedInputStream::_pool(sizeof(HTTPChunkedInputStream));
...
Lucid::MemoryPool HTTPChunkedOutputStream::_pool(sizeof(HTTPChunkedOutputStream));
