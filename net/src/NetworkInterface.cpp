...
std::ostream& operator << (std::ostream& os, const Lucid::Net::NetworkInterface::MACAddress& mac)
{
	std::ios state(0);
	state.copyfmt(os);
	for (unsigned i = 0; i < mac.size(); ++i)
	{
		if (i > 0) os << Lucid::Net::NetworkInterface::MAC_SEPARATOR;
...
class NetworkInterfaceImpl: public Lucid::RefCountedObject
{
public:
...
inline const IPAddress& NetworkInterfaceImpl::address(unsigned index) const
{
	...
	else throw NotFoundException(Lucid::format("No address with index %u.", index));
}
...
const IPAddress& NetworkInterfaceImpl::subnetMask(unsigned index) const
{
	...
	throw NotFoundException(Lucid::format("No subnet mask with index %u.", index));
}
...
const IPAddress& NetworkInterfaceImpl::destAddress(unsigned index) const
{
	...
	throw NotFoundException(Lucid::format("No address with index %u.", index));
}
...
IPAddress subnetMaskForInterface(const std::string& name, bool isLoopback)
{
	...
	else
	{
		...
		HKEY hKey;
		std::wstring usubKey;
		Lucid::UnicodeConverter::toUTF16(subKey, usubKey);
		...
		Lucid::UnicodeConverter::toUTF8(unetmask, netmask);
		RegCloseKey(hKey);
...
NetworkInterface::Map NetworkInterface::map(bool ipOnly, bool upOnly)
{
	...
	Lucid::Buffer<UCHAR> memory(outBufLen);
	...
	for (; pAddress; pAddress = pAddress->Next)
	{
		...
		Lucid::UnicodeConverter::toUTF8(pAddress->FriendlyName, name);
		Lucid::UnicodeConverter::toUTF8(pAddress->Description, displayName);
...
void setInterfaceParams(struct ifaddrs* iface, NetworkInterfaceImpl& impl)
{
	...
	if (addrStream.good())
	{
		...
		Lucid::StringTokenizer tok(addr, ":");
		std::vector<unsigned char> mac;
		for (Lucid::StringTokenizer::Iterator it = tok.begin(); it != tok.end(); ++it)
		{
			mac.push_back(static_cast<unsigned char>(Lucid::NumberParser::parseHex(*it)));
		}
...
NetworkInterface::Map NetworkInterface::map(bool ipOnly, bool upOnly)
{
	...
	return result;
#else
	throw Lucid::NotImplementedException("Not implemented in Android");
#endif
}
