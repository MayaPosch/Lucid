...
Object::Object(int options):
	_preserveInsOrder((options & Lucid::JSON_PRESERVE_KEY_ORDER) != 0),
...
Lucid::dynamicStruct Object::makeStruct(const Object::Ptr& obj)
{
	return makeStructImpl<Lucid::dynamicStruct>(obj);
}


Lucid::OrdereddynamicStruct Object::makeOrderedStruct(const Object::Ptr& obj)
{
	return makeStructImpl<Lucid::OrdereddynamicStruct>(obj);
}
...
{
	if (!_pOrdStruct)
		_pOrdStruct = new Lucid::OrdereddynamicStruct;
	else
		_pOrdStruct->clear();
}
*/
...
{
	if (!_pStruct)
		_pStruct = new Lucid::dynamicStruct;
	else
		_pStruct->clear();
}*/


Object::operator const Lucid::dynamicStruct& () const
{
	if (!_values.size())
	{
	...
	else if (_modified)
	{
		...
		for (; it != end; ++it)
		{
			...
			{
				_pStruct->insert(it->first, Lucid::JSON::Array::makeArray(getArray(it->first)));
			}
			else
			{
...
Object::operator const Lucid::OrdereddynamicStruct& () const
{
	if (!_values.size())
	{
	...
	else if (_modified)
	{
		if (_preserveInsOrder)
		{
			...
			for (; it != end; ++it)
			{
				...
				{
					_pOrdStruct->insert((*it)->first, Lucid::JSON::Array::makeArray(getArray((*it)->first)));
				}
				else
				{
