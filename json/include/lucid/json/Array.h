...
class JSON_API Array
	...
	/// convert Array to Lucid::dynamic::Array. Conversion requires copying and therefore
	/// has performance penalty; the benefit is in improved syntax, eg:
	///
	///    // use pointers to avoid copying
	///    using namespace Lucid::JSON;
	...
	///
	///    // copy/convert to Lucid::dynamic::Array
	///    Lucid::dynamic::Array da = *arr;
	///    i = da[0]["test"];     // i == 0
	///    i = da[1]["test1"][1]; // i == 2
	///    i = da[1]["test2"];    // i == 4
	/// ----
...
public:
	...
	operator const Lucid::dynamic::Array& () const;
		/// Conversion operator to dynamic::Array.

	static Lucid::dynamic::Array makeArray(const JSON::Array::Ptr& arr);
...
private:
	...
	typedef SharedPtr<Lucid::dynamic::Array> ArrayPtr;
