...
using Lucid::OutOfMemoryException;
...
std::string WinService::displayName() const
{
	...
	std::string dispName;
	Lucid::UnicodeConverter::toUTF8(udispName, dispName);
...
void WinService::registerService(const std::string& path, const std::string& displayName)
{
	close();
	std::wstring uname;
	Lucid::UnicodeConverter::toUTF16(_name, uname);
	std::wstring udisplayName;
	Lucid::UnicodeConverter::toUTF16(displayName, udisplayName);
	std::wstring upath;
	Lucid::UnicodeConverter::toUTF16(path, upath);
...
void WinService::setFailureActions(FailureActionVector failureActions, const std::string& command, const std::string& rebootMessage)
{
	...
	Lucid::UnicodeConverter::toUTF16(rebootMessage, urebootMessage);
	...
	Lucid::UnicodeConverter::toUTF16(command, uComamnd);
