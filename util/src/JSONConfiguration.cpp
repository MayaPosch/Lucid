...
{
	Lucid::FileInputStream fis(path);
	load(fis);
}
...
bool JSONConfiguration::getRaw(const std::string & key, std::string & value) const
{
	...
	Lucid::DynamicAny result = query.find(key);
	if ( ! result.isEmpty() )
	{
...
void JSONConfiguration::setValue(const std::string& key, const Lucid::DynamicAny& value)
{
