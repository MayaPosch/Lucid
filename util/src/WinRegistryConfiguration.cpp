...
#include "lucid/Util/WinRegistryConfiguration.h"
...
bool WinRegistryConfiguration::getRaw(const std::string& key, std::string& value) const
{
	...
	if (exists)
	{
		...
		case WinRegistryKey::REGT_DWORD:
			value = Lucid::NumberFormatter::format(aKey.getInt(keyName));
			break;
#if defined(POCO_HAVE_INT64)
		case WinRegistryKey::REGT_QWORD:
			value = Lucid::NumberFormatter::format(aKey.getInt64(keyName));
			break;
#endif
		default:
			exists = false;
...
{
	throw Lucid::NotImplementedException("Removing a key in a WinRegistryConfiguration");
}
