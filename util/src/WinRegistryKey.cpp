...
void WinRegistryKey::setString(const std::string& name, const std::string& value)
{
	open();
	std::wstring uname;
	Lucid::UnicodeConverter::toUTF16(name, uname);
	std::wstring uvalue;
	Lucid::UnicodeConverter::toUTF16(value, uvalue);
...
std::string WinRegistryKey::getStringExpand(const std::string& name)
{
	...
	if (size > 0)
	{
		...
		Lucid::Buffer<wchar_t> expBuffer(expSize);
...
void WinRegistryKey::setInt64(const std::string& name, Lucid::Int64 value)
{
	open();
	std::wstring uname;
	Lucid::UnicodeConverter::toUTF16(name, uname);
...
Lucid::Int64 WinRegistryKey::getInt64(const std::string& name)
{
	open();
	DWORD type;
	Lucid::Int64 data;
...
void WinRegistryKey::deleteKey()
{
	...
	std::wstring usubKey;
	Lucid::UnicodeConverter::toUTF16(_subKey, usubKey);
...
void WinRegistryKey::subKeys(WinRegistryKey::Keys& keys)
{
	...
	for (DWORD i = 0; i< subKeyCount; ++i)
	{
		if (RegEnumKeyExW(_hKey, i, buf, &bufSize, NULL, NULL, NULL, NULL) == ERROR_SUCCESS)
		{
			...
			std::string name;
			Lucid::UnicodeConverter::toUTF8(uname, name);
			keys.push_back(name);
		}
