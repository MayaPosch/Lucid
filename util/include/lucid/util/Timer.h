...
#include "lucid/TimedNotificationQueue.h"
...
class Util_API Timer: protected Lucid::Runnable
...
public:
	...
	explicit Timer(Lucid::Thread::Priority priority);
	...
	void schedule(TimerTask::Ptr pTask, Lucid::Timestamp time);
	...
	void schedule(TimerTask::Ptr pTask, Lucid::Clock clock);
		/// Schedules a task for execution at the specified time.
		///
		/// If the time lies in the past, the task is executed
		/// immediately.
	...
	void schedule(TimerTask::Ptr pTask, Lucid::Timestamp time, long interval);
		/// Schedules a task for periodic execution.
		///
	...
	void schedule(TimerTask::Ptr pTask, Lucid::Clock clock, long interval);
		/// Schedules a task for periodic execution.
		///
		/// The task is first executed at the given time.
		/// Subsequently, the task is executed periodically with
	...
	void scheduleAtFixedRate(TimerTask::Ptr pTask, Lucid::Timestamp time, long interval);
		/// Schedules a task for periodic execution at a fixed rate.
		///
		/// The task is first executed at the given time.
		/// Subsequently, the task is executed periodically
	...
	void scheduleAtFixedRate(TimerTask::Ptr pTask, Lucid::Clock clock, long interval);
		/// Schedules a task for periodic execution at a fixed rate.
		///
		/// The task is first executed at the given time.
		/// Subsequently, the task is executed periodically
