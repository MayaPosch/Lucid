...
#include "lucid/Util/LayeredConfiguration.h"
...
public:
	...
	using SubsystemPtr = Lucid::AutoPtr<Subsystem>;
	...
	const Lucid::Timestamp& startTime() const;
		/// Returns the application start time (UTC).

	Lucid::Timespan uptime() const;
...
protected:
	...
	void setLogger(Lucid::Logger& logger);
	...
	bool findFile(Lucid::Path& path) const;
...
private:
	...
	void getApplicationPath(Lucid::Path& path) const;
	void processOptions();
	bool findAppConfigFile(const std::string& appName, const std::string& extension, Lucid::Path& path) const;
	bool findAppConfigFile(const Path& basePath, const std::string& appName, const std::string& extension, Lucid::Path& path) const;

	typedef LayeredConfiguration::Ptr ConfigPtr;
	typedef Lucid::Logger::Ptr LoggerPtr;
...
template <class C> C& Application::getSubsystem() const
{
	...
	throw Lucid::NotFoundException("The subsystem has not been registered", typeid(C).name());
}
...
inline Lucid::Logger& Application::logger() const
{
...
inline const Lucid::Timestamp& Application::startTime() const
{
	return _startTime;
}


inline Lucid::Timespan Application::uptime() const
{
	Lucid::Timestamp now;
	Lucid::Timespan uptime = now - _startTime;
...
//
	...
	{										\
		Lucid::AutoPtr<App> pApp = new App;	\
		try									\
		{									\
			pApp->init(argc, argv);			\
		}									\
		...
		{									\
			pApp->logger().log(exc);		\
			return Lucid::Util::Application::EXIT_CONFIG;\
		}									\
		return pApp->run();					\
	}
